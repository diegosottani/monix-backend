...
export const delete_user_planned_expense_category = async (req, res) => {
  try {
    ...
    if (!planned_expense_category_id) {
      return res.status(400).json({ error: 'O id da categoria da entrada planejada é obrigatório' });
    }
    ...
    return res.status(200).send("Categoria de despesa planejada excluído com sucesso");
  } catch (error) {
    console.error('Erro ao excluir categoria de despesa planejada:', error);
    return res.status(500).json({ error: 'Erro ao excluir categoria da despesa planejada' });
  }
};
