...
export const get_releases = async (req, res) => {
  try {
    const fields = `id, date, description, account:account_id (name), value, type`;
    const expensesFields = `id, date, description, account:account_id (name), card:card_id (name), value, type`;
    
    const incomings = await getData("incomings", fields, req);
    const expenses = await getData("expenses", expensesFields, req);
