...
export const delete_user_planned_incoming_category = async (req, res) => {
  try {
    ...
    return res.status(200).send("Categoria de entrada planejada excluído com sucesso");
  } catch (error) {
    console.error('Erro ao excluir categoria de entrada planejada:', error);
    return res.status(500).json({ error: 'Erro ao excluir categoria da entrada planejada' });
  }
};
